...
class HybridRAG:
    ...
    def __init__(self, config_path: str = "config.yaml", use_llm: bool = True, variant: str = None, heuristics: str = None):
        ...
        self.num_ctx_tuned = int(_llm_cfg.get("num_ctx", 2048) or 2048)
        ...
        try:
            ...
            if candidate_path:
                ...
                _rr_max_length = int(os.environ.get('RERANKER_MAX_LENGTH', 350))
    ...
    def execute_via_kernel(
        ...
        """Camino Kernel (ADR-0020): Ejecuta Controller y devuelve ExecutionResult."""
        from src.kernel.state import ExecutionResult
    ...
    def query_via_kernel(
        ...
        """Alias shim hacia execute_via_kernel (ADR-0010 / ADR-0020)."""
        exec_res = self.execute_via_kernel(
            question,
            top_k=top_k,
            semantic_weight=semantic_weight,
            use_llm=use_llm,
        ...
        res["method"] = "kernel_linear"
        res.setdefault("memory_hits", 0)
        res.setdefault("timing_breakdown", dict(exec_res.execution_state.timing_ms or {}))
        if "time" not in res or res.get("time") in (None, 0, 0.0):
            res["time"] = float((exec_res.execution_state.timing_ms or {}).get("t_total_s", 0.0) or 0.0)
        return res
    ...
    def _augment_with_page_neighbors(self, results: list, per_source_limit: int = 4) -> list:
        ...
        """Añade páginas vecinas (Â±1) para cada (source,page) presente en results, sin duplicar.
    ...
    def _extract_doc_scope(self, query: str) -> str:
        ...
        """Intenta extraer un nombre de documento para limitar la búsqueda (e.g., "buscar en 'ISO 27001 Guide.pdf'").
    ...
    def _extract_doc_pages_hint(self, query: str):
        ...
        try:
            ...
            doc_match = re.search(r"['\"]([^'\"]+?\.pdf)['\"]|([A-Za-zÁÃ‰ÍÃ“ÃšÃ‘áéíóúñ0-9 \-]+?\.pdf)", q)
    ...
    def _reason_and_retry_search(self, original_query: str, failed_results: list, attempt: int = 1) -> dict:
        ...
        # OPTIMIZACIÃ“N: Solo tomar 2 resultados más relevantes, texto corto
...
Respuesta:"""
    ...
    def _enrich_query_with_context(self, query: str) -> tuple[str, list]:
        ...
        if has_reference and self.last_entities:
            ...
            if prev_entity:
                ...
                words = re.findall(r"\b[\wáéíóúñÁÃ‰ÍÃ“ÃšÃ‘-]+\b", query_lower)
    ...
    def _should_use_conversation_context(self, current_query: str, last_query: str = None) -> bool:
        ...
        if has_continuation:
            ...
            words_c = re.findall(r"\b[\wáéíóúñÁÃ‰ÍÃ“ÃšÃ‘-]+\b", current_lower)

            words_l = re.findall(r"\b[\wáéíóúñÁÃ‰ÍÃ“ÃšÃ‘-]+\b", last_lower)
        ...
        def extract_technical_entities(text):
            ...
            words = re.findall(r'\b[a-záéíóúñA-ZÁÃ‰ÍÃ“ÃšÃ‘][a-záéíóúñA-ZÁÃ‰ÍÃ“ÃšÃ‘0-9-]+\b', text)
    ...
    def _extract_follow_up_anchor(self, user_query: str, last_answer: str) -> str:
        ...
        - Si no hay comillas, buscar una secuencia contigua de â‰¥5 palabras del usuario que aparezca en la última respuesta.
        ...
        try:
            ...
            # 2) Coincidencia contigua de â‰¥5 palabras con la última respuesta

            if la:

                words = re.findall(r'\b\w[\wáéíóúñÁÃ‰ÍÃ“ÃšÃ‘-]*\b', uq)
    ...
    def generate_with_ollama(self, query: str, context: str, conv_context: str = "", detailed: bool = False, is_aggregation: bool = False, num_items: int = 0, is_conceptual: bool = False, is_procedural: bool = False, is_direct_comparison: bool = False, is_simple_numeric: bool = False, is_troubleshooting: bool = False, is_summary: bool = False, length_mode: str = None, stream: bool = False, token_callback=None, cancel_checker=None) -> str:
        ...
        domain_restriction = """RESTRICCIÃ“N CRÍTICA: Eres un asistente técnico especializado en ciberseguridad.
...
- EXCEPCIÃ“N: Si la pregunta pide un dato específico (precio, fecha, contraseña, CVE, versión, cantidad) y los documentos no lo contienen, responde: "No se encontró información en los documentos para esa consulta."

- PROHIBIDO responder en inglés cuando la pregunta está en español. Si los documentos están en inglés, traduce la información al español.
...
"""
        ...
        try:
            ...
            # DEBUG TEMPORAL: tamaños de contexto y prompt

            _ctx_chars = len(context or '')

            _prompt_chars = len(prompt or '')

            _ctx_tokens_approx = _ctx_chars // 4

            _prompt_tokens_approx = _prompt_chars // 4

            _num_ctx_opt = options.get('num_ctx', '?')

            console.print(f"[dim cyan][DEBUG] context={_ctx_chars} chars (~{_ctx_tokens_approx} tok) | prompt={_prompt_chars} chars (~{_prompt_tokens_approx} tok) | num_ctx={_num_ctx_opt}[/dim cyan]")
            ...
            try:
                ...
                console.print(f"[yellow]Â¿Está Ollama corriendo? Error: {str(e)[:100]}[/yellow]")
...
Respuesta:"""
            ...
            # MODO COMPARACIÃ“N DIRECTA: Comparar dos entidades específicas uno-a-uno
...
Respuesta:"""
            ...
            # MODO NUMÃ‰RICO SIMPLE: Respuesta ultra-concisa con un dato numérico
...
Respuesta:"""
            ...
            # MODO TROUBLESHOOTING/DIAGNÃ“STICO: Guía de diagnóstico basada en docs
...
Respuesta:"""
            ...
            # MODO AGREGACIÃ“N: Usar SOLO la tabla extraída

            # Evitar este modo si el usuario pide TECNOLOGÍA o COBERTURA COMPLETA de documentos
...
3. Si encuentras información parcial en un documento, CONTINÃšA buscando en otros fragmentos.
...
2. **COMBINA INFORMACIÃ“N**: Si varios documentos contienen datos relevantes, combínalos.

3. **BUSCA PATRONES NUMÃ‰RICOS**: Tablas, frases con números ("El framework tiene X controles"), números en texto ("diez y seis").
...
Respuesta:"""
                ...
                # MODO NUMÃ‰RICO SIMPLE: Extraer un dato específico (potencia, cantidad, etc.)
...
3. **EXTRAE TODA LA INFORMACIÃ“N RELEVANTE**: Si varios documentos contienen datos sobre la pregunta, COMBINA toda la información.
...
- Prioridad 2: Si los documentos son insuficientes para responder la pregunta, PUEDES complementar con conocimiento general del dominio de ciberseguridad/IT.

- CUANDO uses conocimiento general, marca claramente con prefijo: "[Conocimiento general]" antes de esa parte de la respuesta. El conocimiento general debe ser limitado a definiciones canónicas, expansiones de acrónimos ampliamente aceptados o taxonomías de dominio.

- NUNCA inventes información específica (números de versión, cantidades exactas, nombres de personas, nombres de controles/frameworks particulares, fechas) que no esté explícitamente en los documentos o no sea de dominio público verificable.

- Si una pregunta requiere datos exactos y estos no están en los documentos, indica: "[Conocimiento general] Según conocimiento del dominio, [tu respuesta basada en conocimiento general]." No cites documentos falsos para esos datos.

- No contradigas el contexto: si un documento dice "X" y tu conocimiento general dice "Y", usa "X" y, si es relevante, añade "[Conocimiento general] Algunas fuentes generales mencionan Y."
    ...
    def _build_ollama_options(self, detailed, length_mode, query, is_listing):
        ...
        if detailed:
            ...
            options = {
                ...
                "num_ctx": getattr(self, 'num_ctx_tuned', 2048),
...
"""
        ...
        # Comando desconocido
            ...
            response += "â€¢ `/memoria` - Ver sinónimos guardados\n"
    ...
    def execute(self, question: str, top_k: int = 50,

              semantic_weight: float = 0.6, use_llm: bool = None,

              entity_filter: bool = None, two_stage: bool = None,

              length_mode: str = None, no_context: bool = False,

              stream: bool = False, token_callback=None, docs_callback=None, cancel_checker=None,

              return_prerank: bool = False):

        """

        Contrato unico de ejecucion del sistema (ADR-0020).

        Devuelve ExecutionResult con answer, sources y execution_state.

        """

        from src.kernel.state import ExecutionResult, LinearStateAdapter



        if entity_filter is not None or two_stage is not None:

            import warnings

            warnings.warn(

                "Los parametros 'entity_filter' y 'two_stage' estan deprecados (ADR-0020). "

                "La estrategia de retrieval la decide unicamente el pipeline.",
        ...
        eff_entity_filter = True if entity_filter is None else bool(entity_filter)

        eff_two_stage = True if two_stage is None else bool(two_stage)
        ...
        if getattr(self, "kernel_enabled", False):
            ...
            return self.execute_via_kernel(
        ...
        res_dict = self._query_linear_impl(
            ...
            entity_filter=eff_entity_filter,

            two_stage=eff_two_stage,
        ...
        state = LinearStateAdapter.build_state(question, res_dict)

        return ExecutionResult(

            answer=res_dict.get("answer", "") or "",
    ...
    def query(self, question: str, top_k: int = 50,
        ...
        Fachada congelada de consulta (ADR-0010 / ADR-0020). Shim de compatibilidad.

        Todo desarrollo nuevo debe utilizar HybridRAG.execute().
        ...
        return exec_res.to_query_result()
    ...
    def _query_linear_impl(self, question: str, top_k: int = 50,
        ...
        try:
            ...
            if self.flags.get('strict_ood', True) and self._is_out_of_domain(question):
                ...
                brief = (
                    ...
                    "â€¢ Tecnologías (firewalls, SIEM, EDR, cloud security)\n"

                    "â€¢ Metodologías (pentesting, red team, blue team, DevSecOps)\n"
        ...
        if is_doc_explanation and doc_ref:
            ...
            console.print(f"[dim]ðŸ“„ Explicación de documento detectada: {doc_ref['doc_name']} (página {doc_ref['page']})[/dim]")
                ...
                try:
                    ...
                    if len(entities) >= 1:
                        ...
                        try:
                            ...
                            # EXCEPCIÃ“N 1: Si es comparación, NO limpiar sticky sources (necesitamos ambas entidades)
        ...
        # BÃšSQUEDA EN DOS ETAPAS (solo si no es comparación o explicación de doc)
        ...
        if results is None and two_stage and entities and len(entities) <= 2 and not is_procedural and not locals().get('is_listing_ctx', False) and not self._is_sum_query(question) and not self._extract_tech_filter(question) and not self._extract_vendor_filter(question) and not pt_like:
            ...
            # Etapa 1: Buscar primero en documento especifico específico si existe
            ...
            if target_anexo:
                ...
                # Mejor heurística: tomar cualquier 'documento especifico' cuyo TEXTO mencione la entidad
        ...
        if results and not is_aggregation and self.flags.get('boost_source_by_entity', True):
            ...
            for r in results:
                ...
                # Boost: documento especifico solo si hay mención explícita de la entidad
        ...
        if results and not is_aggregation and not is_detailed and not (is_comparison and self.flags.get('skip_quality_filter_for_comparison', True)) and not locals().get('is_listing_ctx', False) and not locals().get('doc_scope', ''):
            ...
            if quality_results:
                ...
                # EXCEPTO cuando el usuario pidió revisar TODOS los documentos / cobertura completa
        ...
        # OPTIMIZACIÃ“N: Deduplicación semántica de resultados
        ...
        # OPTIMIZACIÃ“N: Limitar resultados por fuente (máx 2 por documento)
        ...
        if is_aggregation:
            ...
            # Si se pide tecnología o cobertura completa, FORZAR uso de documentos individuales
            ...
            if not results:
                ...
                if not source_best_score:
                    ...
                    # PRIORIDAD 1: Si Etapa 1 encontró un documento especifico específico, usarlo
                    ...
                    # PRIORIDAD 2: Buscar si hay un documento especifico específico en los top-5 resultados
                    ...
                    # DECISIÃ“N INTELIGENTE: Â¿Necesita múltiples documentos?
                    ...
                    # EXCEPCIÃ“N: Si Etapa 1 encontró un documento especifico específico, NO usar multi-doc (usar solo ese)
                    ...
                    for i, source in enumerate(all_docs['metadatas']):
                        ...
                        if doc_source in selected_sources:
                            ...
                            entity_score = 0.0
                            entity_mode = self.config.get('retrieval', {}).get('entity_mode', 'boost')
                            if entity_filter_active and entity_mode != 'off':
                    ...
                    # NUNCA priorizar documento de listado salvo que sea una query de listado explícito
                    ...
                    for source in ordered_sources:
                        ...
                        # FILTRAR chunks con score muy bajo (< 1.0) solo si entity_mode es 'filter' (ADR-0020)
                        entity_mode = self.config.get('retrieval', {}).get('entity_mode', 'boost')
                        if entity_filter_active and entity_mode == 'filter':
        ...
        # OPTIMIZACIÃ“N FASE 3: Usar contexto estructurado por categorías para consultas normales
        ...
        if use_llm:
            ...
            if not answer:
                ...
                # Inyección de FOCUS detallado deshabilitada por defecto (evita secciones tipo "Cantidad de elementos")
    ...
    def display_result(self, result: dict, show_details: bool = True):
        ...
        if show_details:
            ...
            console.print(f"\n[bold yellow]ðŸ“Š Top-5 Fragmentos (scores híbridos):[/bold yellow]")
    ...
    def interactive_mode(self):
        ...
        while True:
            ...
            try:
                ...
                result = self.execute(question).to_query_result()
...
def test_hybrid_rag():
    ...
    for semantic_weight in [0.3, 0.5, 0.7]:
        ...
        result = rag.execute(question, top_k=25, semantic_weight=semantic_weight, use_llm=False).to_query_result()
    ...
    result = rag.execute(question, top_k=30, semantic_weight=0.5).to_query_result()
